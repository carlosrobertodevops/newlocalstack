...
class SQSQueueProvider(SQSQueueProviderBase):
    ...
    def _autogenerated_queue_name(self, request: ResourceRequest[SQSQueueProperties]) -> str:
        ...
        if self._is_true_property(request.desired_state.get("FifoQueue")):
    ...
    def _populate_missing_attributes_with_defaults(self, properties: SQSQueueProperties) -> None:
        ...
        if self._is_true_property(properties.get("FifoQueue")):
            for k, v in self.DEFAULT_FIFO_ATTRIBUTE_VALUES.items():
                properties.setdefault(k, v)

    def _is_true_property(self, property_value: str | None) -> bool:
        """
        Detect whether a 'true' value is passed in a property. If it's None (property was omitted) or False, or any
        type of string (e.g. a typo such as "Fasle"), then it's not a true value. This extra check is needed because
        the CloudFormation engine doesn't fully validate booleans properties before passing them to the resource provider.
        """
        return (
            property_value == True  # noqa: E712
            or (isinstance(property_value, str) and property_value.lower() == "true")
        )
